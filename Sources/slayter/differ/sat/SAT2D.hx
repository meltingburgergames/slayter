...
class SAT2D {
	...
	static inline function findNormalAxisX(verts:Array<DifferVec>, index:Int):Float {
		var v2 = (index >= verts.length - 1) ? verts[0] : verts[index + 1];
		return -(v2.y - verts[index].y);
	}

	static inline function findNormalAxisY(verts:Array<DifferVec>, index:Int):Float {
		var v2 = (index >= verts.length - 1) ? verts[0] : verts[index + 1];
